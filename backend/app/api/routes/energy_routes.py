...
def get_service():
    return EnergyService.get_instance()

@energy_bp.route("/telemetry", methods=["POST"])
def post_telemetry():
    """
...
@energy_bp.route("/settings/thresholds", methods=["POST"])
...
@energy_bp.route("/top-devices", methods=["GET"])
def top_devices():
    service = get_service()
...
@energy_bp.route("/all", methods=["GET"])
def all_data():
    service = get_service()
